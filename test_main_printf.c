...
int main()
{
	...
	printf(" || byte:%d\n", ft_printf("%p %p %p %p", "test", p, "bonjour", p));
	printf(" || byte:%d\n", printf("%p %p %p %p", "test", p, "bonjour",p));

	write(1,"\n",1);

	char *test = "grostasdemorve";
	printf("%s %p\n", test, test);
	uintptr_t ptrtest = (uintptr_t)test;
	printf("%lu\n", ptrtest);
	ft_putpointer(test);
	printf("\n%s\n", (char *)ptrtest);
	ptrtest = ptrtest + 4;
	...
	printbyte = ft_printf("%x %x %x %x %x %x\n", LONG_MAX, INT_MAX, INT_MIN, LONG_MIN, ULONG_MAX, INT_MAX);
	printf("printbyte test : %d\n\n", printbyte);
